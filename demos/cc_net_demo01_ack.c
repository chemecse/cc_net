...
void client(void)
{
	...
	printf("[client] sending packet with a value of %d\n", packet.value);
	...
	while (is_running) {
        ...
        if (recv_data_size < 0) {
			...
			printf("[client] recv'd packet with a value of %d\n", packet.value);
...
#define MAX_CLIENT_COUNT 256
    ...
    while (is_running) {
        ...
        if (recv_data_size < 0) {
			...
			printf("[server] recv'd packet with a value of %d\n", packet.value);
			...
			if (packet.value == 666) {
				...
				packet.header = header;
				packet.value += 1;
				printf("[server] sending packet with a value of %d\n", packet.value);
